...
const SHOP_ADMIN_EMAIL_BY_SLUG: Record<string, string> = {
  'twins-bros': 'admin@twins-bros.gr',
  salut: 'admin@salut.gr',
}
...
function appendShopParam(path: string, shopSlug: string) {
  const separator = path.includes('?') ? '&' : '?'
  return `${path}${separator}shop=${encodeURIComponent(shopSlug)}`
}

function getShopName(shopSlug: string) {
  return SHOP_NAME_BY_SLUG[shopSlug] ?? 'Twins Bros'
}

function getShopAdminEmail(shopSlug: string) {
  return SHOP_ADMIN_EMAIL_BY_SLUG[shopSlug] ?? 'admin@twins-bros.gr'
}

function getShopLogo(shopSlug: string) {
  return SHOP_LOGO_BY_SLUG[shopSlug] ?? '/logo.webp'
}

function getShopTheme(shopSlug: string) {
  return SHOP_THEME_BY_SLUG[shopSlug] ?? SHOP_THEME_BY_SLUG['twins-bros']
}
...
function StatusBadge({ status }: { status: ReturnType<typeof getBookingStatus> }) {
  const styles = {
    ...
    cancelled: 'border-[var(--admin-accent)]/50 bg-[var(--admin-accent)]/15 text-[var(--admin-accent-soft)]',
  }
  const labels = {
...
const primaryButtonClass =
  'rounded-xl bg-[var(--admin-accent)] px-5 py-3 text-sm font-black text-white shadow-lg shadow-[var(--admin-accent)]/20 transition hover:bg-[var(--admin-accent-hover)] disabled:cursor-not-allowed disabled:opacity-50'
...
function redirectToAdminLogin(shopSlug = DEFAULT_SHOP_SLUG) {
  window.location.assign(
    shopSlug === DEFAULT_SHOP_SLUG
      ? '/admin/login'
      : apiPath('/admin/login', shopSlug),
  )
}
...
function SectionHeader({ eyebrow, title }: { eyebrow: string; title: string }) {
  return (
    <div className="mb-5">
      <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--admin-accent)]">{eyebrow}</p>
      ...
      {customers.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-2xl border border-white/10">
          <table className="w-full min-w-[640px] text-left text-sm">
            <thead>
              <tr className="bg-[var(--admin-surface)]/60 text-xs font-black uppercase tracking-[0.14em] text-zinc-400">
            ...
            <tbody>
              {customers.map((customer) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right font-black text-[var(--admin-accent-soft)]">{customer.visits}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function ErrorBanner({ message }: { message: string }) {
  return (
    <p className="mb-4 rounded-xl border border-[var(--admin-accent)]/40 bg-[var(--admin-accent)]/10 p-3 text-sm text-[var(--admin-accent-muted)]">
      {message}
    </p>
  )
}
    ...
    return (
      <div className="flex shrink-0 items-center gap-2">
        <button
          ...
          onClick={onConfirm}
          disabled={busy}
          className="rounded-xl bg-[var(--admin-accent)] px-3 py-2 text-xs font-black text-white transition hover:bg-[var(--admin-accent-hover)] disabled:opacity-50"
        >
          {busy ? '...' : 'Διαγραφή'}
        </button>
        <button
  ...
  return (
    ...
    >
      <form
        ...
        className="relative max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl border border-[var(--admin-deep)] bg-[var(--admin-bg)] p-6 shadow-2xl shadow-[var(--admin-accent)]/15"
      >
        <button
          type="button"
          onClick={onClose}
          aria-label="Κλείσιμο"
          className="absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-xl border border-[var(--admin-accent)]/30 bg-[var(--admin-accent)]/10 text-[var(--admin-accent-soft)] transition hover:bg-[var(--admin-accent)]/20 hover:text-white"
        >
          <XIcon className="h-4 w-4" />
        </button>
        <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--admin-accent)]">
          {shopName}
        </p>
        <h2 className="mt-1 text-xl font-black text-white sm:text-2xl">{title}</h2>
  ...
  async function handleDelete(id: string) {
    ...
    try {
      const res = await fetch(
        appendShopParam(`/api/barbers?id=${encodeURIComponent(id)}`, shopSlug),
        {
        method: 'DELETE',
        },
      )
      ...
      {barbers.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {barbers.map((barber) => (
            <div
              key={barber.id}
              className="flex items-center gap-4 rounded-2xl border border-white/10 bg-[var(--admin-surface-2)]/80 p-4"
            >
              <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-full border border-[var(--admin-accent)]/30 bg-black">
                {barber.image ? (
        ...
        >
          ...
          <label className="block">
            ...
            <input
              ...
              className="mt-2 w-full rounded-xl border border-[var(--admin-deep)] bg-black/70 px-4 py-3 text-sm text-zinc-300 outline-none transition file:mr-4 file:rounded-lg file:border-0 file:bg-[var(--admin-accent)] file:px-3 file:py-2 file:text-xs file:font-black file:text-white hover:file:bg-[var(--admin-accent-hover)] focus:border-[var(--admin-accent)] focus:ring-2 focus:ring-[var(--admin-accent)]/25"
            />
          </label>
          {(imagePreview || imageProcessing) && (
            <div className="flex items-center gap-4 rounded-xl border border-white/10 bg-black/35 p-3">
              <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-full border border-[var(--admin-accent)]/30 bg-black">
                {imagePreview ? (
                  <Image
                    src={imagePreview}
          ...
          {modalError && (
            <p className="rounded-xl border border-[var(--admin-accent)]/40 bg-[var(--admin-accent)]/10 p-3 text-sm text-[var(--admin-accent-muted)]">
              {modalError}
            </p>
          )}
        </ModalShell>
...
function ServicesView({
  ...
  async function handleDelete(id: string) {
    ...
    try {
      const res = await fetch(
        appendShopParam(`/api/services?id=${encodeURIComponent(id)}`, shopSlug),
        { method: 'DELETE' },
      )
      if (!res.ok) {
        const data = (await res.json().catch(() => null)) as
        ...
        <div className="space-y-3">
          {services.map((service) => (
            ...
            >
              ...
              <p className="shrink-0 text-lg font-black tabular-nums text-[var(--admin-accent-soft)]">
  ...
  return (
    <section className={cardClass}>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-[var(--admin-accent)]/30 bg-[var(--admin-surface)]/60 p-6">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--admin-accent-soft)]">
            Έσοδα Σήμερα
          </p>
      ...
      <div className="mt-6">
        <p className="mb-3 text-xs font-black uppercase tracking-[0.18em] text-[var(--admin-accent)]">
          Απόδοση Κουρέων
        </p>
        {barbers.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-white/15 bg-black/25 p-12 text-center text-sm text-zinc-400">
          ...
          <div className="overflow-x-auto rounded-2xl border border-white/10">
            <table className="w-full min-w-[480px] text-left text-sm">
              ...
              <tbody>
                {stats.perBarber.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right font-black tabular-nums text-[var(--admin-accent-soft)]">
              ...
              <tfoot>
                <tr className="border-t border-white/10 bg-[var(--admin-surface-2)]/80 text-white">
...
function ScheduleManagerView({
  ...
  const loadExceptions = useCallback(async () => {
    ...
    setError(false)
    try {
      const res = await fetch(apiPath('/api/admin/schedule-exceptions', shopSlug))
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await fetch(apiPath(`/api/admin/schedule-exceptions/${id}`, shopSlug), {
        method: 'DELETE',
      })
    } finally {
      await loadExceptions()
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <section className={cardClass}>
          ...
          <div className="space-y-4">
            ...
            <label className="block">
              ...
              <select
                ...
                className={fieldClass}
              >
                <option value="" className="bg-[var(--admin-bg)]">Όλο το μαγαζί</option>
                {barbers.map((barber) => (
                  <option key={barber.id} value={barber.name} className="bg-[var(--admin-bg)]">
                    {barber.name}
                  </option>
                ))}
              </select>
            ...
            {blockError && (
              <p className="text-sm text-[var(--admin-accent-soft)]">{blockError}</p>
            )}
            <button
              type="button"
          ...
          <div className="mt-6">
            ...
            <div className="mt-3 space-y-2">
              {loading ? (
                <div className="h-16 animate-pulse rounded-xl bg-[var(--admin-surface-2)]" />
        ...
        <section className={cardClass}>
          ...
          <div className="space-y-4">
                ...
                <button
                  ...
                  className={`rounded-xl border px-4 py-3 text-sm font-black transition ${
                    overrideMode === 'FORCE_CLOSE'
                      ? 'border-[var(--admin-accent)]/50 bg-[var(--admin-accent)]/15 text-[var(--admin-accent-soft)]'
                      : 'border-white/10 bg-white/[0.04] text-zinc-400 hover:text-white'
                  }`}
                >
                  ΚΛΕΙΣΤΟ
            ...
            {overrideError && (
              <p className="text-sm text-[var(--admin-accent-soft)]">{overrideError}</p>
            )}
            <button
              type="button"
      ...
      {error && (
        <div className="rounded-2xl border border-[var(--admin-accent)]/35 bg-[var(--admin-accent)]/10 p-4 text-center text-sm text-[var(--admin-accent-muted)]">
    ...
    try {
      const res = await fetch(apiPath('/api/bookings', shopSlug), {
        ...
        body: JSON.stringify({
          ...
          customerEmail: customerEmail.trim() || getShopAdminEmail(shopSlug).replace('admin@', 'manual@'),
          adminManual: true,
        }),
      })
  ...
  return (
    ...
    >
      <form
        ...
        onClick={(event) => event.stopPropagation()}
        className="relative max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-[var(--admin-deep)] bg-[var(--admin-bg)] p-6 shadow-2xl shadow-[var(--admin-accent)]/15"
      >
        <button
          type="button"
          onClick={onClose}
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <label className="block">
            ...
            <input
              ...
              placeholder={getShopAdminEmail(shopSlug).replace('admin@', 'manual@')}
              className={fieldClass}
            />
          </label>
        ...
        {error && (
          <p className="mt-4 rounded-xl border border-[var(--admin-accent)]/40 bg-[var(--admin-accent)]/10 p-3 text-sm text-[var(--admin-accent-muted)]">
            {error}
          </p>
        )}
...
function SecuritySettingsView({ shopSlug }: { shopSlug: string }) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      const res = await fetch(apiPath('/api/admin/password', shopSlug), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <section className="rounded-2xl border border-zinc-200 bg-white p-6 text-zinc-950 shadow-2xl shadow-black/15">
      <h2 className="text-xl font-black tracking-wide text-[var(--admin-accent)]">
      ...
      <form onSubmit={(event) => void handleSubmit(event)} className="mt-6 max-w-xl space-y-4">
        ...
        <button
          type="submit"
          disabled={submitting}
          className="rounded-xl bg-[var(--admin-accent)] px-5 py-3 text-sm font-black text-white shadow-lg shadow-[var(--admin-accent)]/20 transition hover:bg-[var(--admin-accent-hover)] disabled:cursor-not-allowed disabled:opacity-50"
        >
...
export default function AdminDashboard({
  ...
  // time this renders, the visitor already holds a valid admin session, and the
  // active shop is read from that session cookie server-side (see page.tsx) —
  // never from the URL — so a missing `?shop=` param can no longer silently
  // fall back to the default shop.
  const [shopSlug] = useState(initialShopSlug)
  const shopReady = true
  ...
  const loadData = useCallback(async (signal?: AbortSignal) => {
    ...
    fetch(apiPath('/api/admin/stats', shopSlug), { signal })
      .then((res) => {
        if (res.status === 401) redirectToAdminLogin(shopSlug)
    ...
    fetch(apiPath('/api/services', shopSlug), { signal })
    ...
    fetch(apiPath('/api/barbers', shopSlug), { signal })
    ...
    try {
      const bookingsRes = await fetch(apiPath('/api/admin/bookings', shopSlug), { signal })
      if (bookingsRes.status === 401) {
        redirectToAdminLogin(shopSlug)
        return
      }
  ...
  async function handleCancel(id: string) {
    setCancelling(true)
    try {
      await fetch(apiPath(`/api/admin/bookings/${id}`, shopSlug), {
        method: 'DELETE',
      })
  ...
  async function handleHardDelete(id: string) {
    ...
    try {
      const res = await fetch(apiPath(`/api/admin/bookings/${id}`, shopSlug), {
        method: 'DELETE',
      })
  ...
  async function handleNoShowToggle(booking: Booking) {
    ...
    try {
      const res = await fetch(apiPath(`/api/admin/bookings/${booking.id}`, shopSlug), {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const shopName = getShopName(shopSlug)
  const shopAdminEmail = getShopAdminEmail(shopSlug)
  const shopLogo = getShopLogo(shopSlug)
  const shopTheme = getShopTheme(shopSlug)
    ...
    >
      <div className="mx-auto max-w-7xl">
        <header className="mb-4 flex flex-col gap-4 rounded-2xl border border-white/10 bg-black/35 p-5 shadow-2xl shadow-black/30 backdrop-blur lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-4">
            <Image src={shopLogo} alt={shopName} width={58} height={58} className="rounded-full border border-white/15 bg-black object-cover shadow-lg shadow-[var(--admin-accent)]/10" priority />
            <div>
              <p className="text-xs font-black uppercase tracking-[0.26em] text-[var(--admin-accent)]">{shopName}</p>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-2 text-sm text-zinc-300">
              <span className="text-zinc-500">Συνδεδεμένος:</span> {shopAdminEmail}
            </div>
            <button
              type="button"
              onClick={handleLogout}
              className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-zinc-200 transition hover:border-[var(--admin-accent)]/50 hover:bg-[var(--admin-accent)]/10"
            >
              Αποσύνδεση
            </button>
          </div>
            ...
            return (
              <button
                ...
                className={`shrink-0 rounded-xl px-4 py-2.5 text-sm font-black transition ${
                  isActive
                    ? 'bg-[linear-gradient(135deg,var(--admin-deep),var(--admin-accent))] text-white shadow-lg shadow-[var(--admin-accent)]/20'
                    : 'bg-white/[0.04] text-zinc-400 hover:bg-[var(--admin-deep)]/70 hover:text-white'
                }`}
              >
                {tab.label}
              </button>
          ...
          <div className="grid gap-5 lg:grid-cols-[minmax(280px,0.95fr)_minmax(0,2fr)]">
            <aside className="space-y-4">
              <section className="rounded-2xl border border-white/10 bg-black/35 p-4 shadow-xl shadow-black/20">
                <div className="mb-4 flex items-center justify-between gap-3">
                  <button
                    ...
                    className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-zinc-200 hover:bg-[var(--admin-deep)]"
                    ...
                    return (
                      <button
                        ...
                        className={`flex aspect-square min-h-9 items-center justify-center rounded-full text-sm font-black transition ${
                          selected
                            ? 'bg-[var(--admin-accent)] text-white shadow-[0_12px_30px_rgba(255,31,45,0.28)]'
                            : currentMonth
                              ? 'text-zinc-200 hover:bg-[var(--admin-deep)] hover:text-white'
              ...
              <section className="grid grid-cols-2 gap-3 lg:grid-cols-1">
                <div className="rounded-2xl border border-white/10 bg-[var(--admin-surface-2)]/80 p-4">
            ...
            <section className="min-w-0 rounded-2xl border border-white/10 bg-black/30 p-4 shadow-2xl shadow-black/25 sm:p-5">
              <div className="mb-5 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--admin-accent)]">Ραντεβού</p>
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                  <label className="flex items-center gap-2 rounded-xl border border-white/10 bg-black/35 px-3 py-2">
                    ...
                    >
                      <option value={ALL_BARBERS} className="bg-[var(--admin-bg)] text-white">
                        {ALL_BARBERS}
                      </option>
                      {barbers.map((barber) => (
                        <option key={barber.id} value={barber.name} className="bg-[var(--admin-bg)] text-white">
                          {barber.name}
                        </option>
                      ))}
                    </select>
                  ...
                  <button
                    ...
                    className="rounded-xl bg-[var(--admin-accent)] px-5 py-3 text-sm font-black text-white shadow-lg shadow-[var(--admin-accent)]/20 transition hover:bg-[var(--admin-accent-hover)]"
                  >
                    + Νέο ραντεβού
                  </button>
                </div>
              ...
              {loading ? (
                <div className="space-y-3">
                  ...
                  {Array.from({ length: 5 }).map((_, index) => (
                    <div key={index} className="h-28 animate-pulse rounded-2xl bg-[var(--admin-surface-2)]" />
                  ))}
                </div>
              ) : error ? (
                <div className="rounded-2xl border border-[var(--admin-accent)]/35 bg-[var(--admin-accent)]/10 p-8 text-center">
                  <p className="text-sm text-[var(--admin-accent-muted)]">Κάτι πήγε στραβά.</p>
                  <button type="button" onClick={() => void loadData()} className="mt-4 rounded-xl border border-[var(--admin-accent)]/50 px-4 py-2 text-sm font-semibold text-white hover:bg-[var(--admin-accent)]/15">
                    Δοκίμασε ξανά
                  </button>
                </div>
                  ...
                  {filteredBookings.map((booking) => {
                    ...
                    return (
                      <article key={booking.id} className="grid gap-4 rounded-2xl border border-white/10 bg-[#080607] p-4 shadow-xl shadow-black/20 md:grid-cols-[96px_minmax(0,1fr)_auto] md:items-center">
                        <div className="flex h-20 items-center justify-center rounded-2xl border border-[var(--admin-accent)]/30 bg-[var(--admin-surface)] font-mono text-2xl font-black text-white shadow-inner shadow-black/30">
                        ...
                        <div className="flex flex-col gap-3 md:items-end">
                          ...
                          <div className="flex flex-wrap gap-2 md:justify-end">
                            {status === 'completed' && (
                              <button
                                ...
                                className={`inline-flex h-10 items-center gap-2 rounded-xl border px-3 text-xs font-black transition disabled:cursor-not-allowed disabled:opacity-60 ${
                                  booking.noShow
                                    ? 'border-red-950 bg-[#5b0b16] text-white shadow-lg shadow-red-950/25 hover:bg-[var(--admin-deep)]'
                            ...
                            <button
                              ...
                              className="inline-flex h-10 items-center gap-2 rounded-xl border border-[var(--admin-accent)]/25 bg-[var(--admin-deep)] px-3 text-xs font-black text-[var(--admin-accent-muted)] transition hover:bg-[#6d0a14]"
                            >
                              <XIcon className="h-4 w-4" />
                              ΑΚΥΡΩΣΗ
                            </button>
                            <button
                              ...
                              className="inline-flex h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 text-xs font-black text-zinc-400 transition hover:border-[var(--admin-accent)]/40 hover:bg-[var(--admin-deep)]/40 hover:text-[var(--admin-accent-soft)]"
        ...
        {activeTab === 'schedule' && (
          <ScheduleManagerView barbers={barbers} shopSlug={shopSlug} />
        )}
        {activeTab === 'revenue' && (
          <RevenueView bookings={bookings} barbers={barbers} />
        )}
        {activeTab === 'settings' && <SecuritySettingsView shopSlug={shopSlug} />}
      </div>
      ...
      {cancelId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-6">
          <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-[var(--admin-bg)] p-6 text-center shadow-2xl shadow-[var(--admin-accent)]/10">
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button
                ...
                disabled={cancelling}
                className="flex-1 rounded-xl bg-[var(--admin-accent)] py-3 text-sm font-black text-white hover:bg-[var(--admin-accent-hover)] disabled:opacity-50"
              >
